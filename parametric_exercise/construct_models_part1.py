...
fitresult = model.fitTo(mc,ROOT.RooFit.SumW2Error(True), Save=True)
fitresult.Print()
...
fit_result = model.fitTo(mc,ROOT.RooFit.SumW2Error(True),ROOT.RooFit.PrintLevel(-1), Save=True)
fit_result.Print()
...
fit_result = model_bkg.fitTo( data, ROOT.RooFit.Range(fit_range),
    ROOT.RooFit.PrintLevel(-1), Save=True
    )
fit_result.Print()
...
# norm.setVal(data.numEntries()*2) # FIXME
# alpha.setVal(2*alpha.getVal()) # FIXME
getattr(w_bkg, "import")(data)
getattr(w_bkg, "import")(norm)
getattr(w_bkg, "import")(model_bkg)
w_bkg.Print()
if post_fit:
    keep_floating = ["CMS_hgg_mass"]
    freeze_all_vars(w_bkg, make_exception=keep_floating)
w_bkg.Write()
f_out.Close()
...
getattr(w_sig, "import")(mc)

w_sig.Print()
if post_fit:
    keep_floating = ["CMS_hgg_mass"]
    freeze_all_vars(w_sig, make_exception=keep_floating)
w_sig.Write()
f_out.Close()
...
# f_out.Close()
# norm.Print("v")
# print(f"bkg norm values: {norm.getVal()}")
# print("----------------------")

print_workspace_vars(w_bkg)
print_workspace_vars(w_sig)